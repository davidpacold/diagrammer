...
# Default includes a pre-configured GPU pool with 0 nodes (scale up when needed)
...
variable "openai_location" {
  type        = string
  description = "Azure region for OpenAI service deployment (limited regions available)"
  default     = "" # Uses resource group location if empty
}
...
variable "key_vault_id" {
  type        = string
  description = "ID of existing Key Vault for storing secrets"
  default     = ""
}
...
variable "document_intelligence_location" {
  type        = string
  description = "Location for Azure Document Intelligence service (if empty, uses resource group location)"
  default     = ""
}

variable "document_intelligence_sku" {
  type        = string
  description = "SKU for Document Intelligence service (F0=free tier, S0=standard)"
  default     = "S0"
}
...
variable "document_intelligence_allowed_ips" {
  type        = list(string)
  description = "List of allowed IP addresses for Azure Document Intelligence"
  default     = []
}
...
variable "ml_workspace_enabled" {
  type        = bool
  description = "Enable Azure Machine Learning workspace deployment for serverless endpoints"
  default     = true
}

variable "ml_workspace_location" {
  type        = string
  description = "Azure region for ML workspace deployment (if empty, uses resource group location)"
  default     = ""
}

variable "ml_workspace_public_network_access_enabled" {
  type        = bool
  description = "Enable public network access for ML workspace (required for Basic Container Registry SKU)"
  default     = false
}

variable "ml_workspace_allowed_ips" {
  type        = list(string)
  description = "List of allowed IP addresses for Azure ML workspace"
  default     = []
}

variable "ml_workspace_private_endpoint_enabled" {
  type        = bool
  description = "Enable private endpoint for ML workspace"
  default     = true
}
...
variable "ml_workspace_storage_replication_type" {
  type        = string
  description = "Storage replication type for ML workspace (LRS, GRS, ZRS, GZRS)"
  default     = "LRS"
}
